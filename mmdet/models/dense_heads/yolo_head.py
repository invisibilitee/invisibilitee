...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
                 ...
                 loss_tv=dict(type='TVLoss'), 
                 loss_nps=dict(type='NPSLoss'), 
                 train_cfg=None,
                 test_cfg=None):
        ...
        self.loss_tv = build_loss(loss_tv) #yaxian
        self.loss_nps = build_loss(loss_nps) #yaxian
    ...
    def loss(self,
        ...
        #yaxian
        patch = patch.repeat(repeats=(len(pred_maps),1,1,1))
        #print('patch.size() in yolo head loss',patch.size())
        losses_cls, losses_conf, losses_xy, losses_wh, loss_tv, loss_nps = multi_apply(
            self.loss_single, pred_maps, target_maps_list, neg_maps_list, patch) 
        ...
        return dict(
            ...
            loss_xy=losses_xy,
            loss_wh=losses_wh,
            loss_tv=loss_tv, #yaxian
            loss_nps=loss_nps) #yaxian

    def loss_single(self, pred_map, target_map, neg_map, patch):
        ...
        Returns:
            tuple:
                ...
                loss_tv (Tensor): Loss of patch. #newly added by yaxian
        """
        ...
        #print('patch.size() in yolo single',patch.size())
        loss_tv = self.loss_tv(patch)
        loss_nps = self.loss_nps(patch)

        return loss_cls, loss_conf, loss_xy, loss_wh, loss_tv, loss_nps
