...
class HourglassNet2(nn.Module):
    ...
    def __init__(self,
        ...
        self.warped_patch = None
        if training:
            self.patch = nn.Parameter(torch.rand(1, 3, 416, 416), requires_grad=True)
        else:
            # patch is path
            print('load patch from path: {}'.format(patch_path))
    ...
    def forward(self, x, gt_keypoints=None):
        ...
        # yaxian
        # print('x.size()', x.size())
        # print('gt_keypoints.size()', len(gt_keypoints))

        for k in range(x.size(0)):
            warped_patch = self.warp_patch(x[k], self.patch, gt_keypoints[k]) #yaxian
            # print('gt_keypoints[k] before add patch', gt_keypoints[k])
