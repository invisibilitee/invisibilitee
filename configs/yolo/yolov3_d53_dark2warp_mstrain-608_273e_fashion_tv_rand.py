...
# attack settings
# model settings
# patch_saved_path = '/home/yaxian_li/expriment/mmdetection-yax/pattern.png'
model = dict(
    type='YOLOV3',
    ...
    bbox_head=dict(
        ...
        loss_xy=dict(
            ...
            loss_weight=2.0,
            reduction='sum'),
        loss_wh=dict(type='MSELoss', loss_weight=2.0, reduction='sum', negative=True),
        loss_tv=dict(type='TVLoss'),
        loss_nps=dict(type='NPSLoss',patch_size=416)))
...
train_pipeline = [
    ...
    dict(type='Resize', img_scale=[(416, 416), (416, 416)], keep_ratio=False),
...
test_pipeline = [
    ...
    # dict(type='LoadAnnotations', with_bbox=True), # yaxian
    dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'annos_attack/deepfashion2_train.json',
        img_prefix=data_root + 'train/image/',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        samples_per_gpu=8,
        ann_file=data_root + 'annos_attack/deepfashion2_val.json',
...
lr_config = dict(
    policy='step',
    warmup='linear',
    warmup_iters=200,  # same as burn-in in darknet
